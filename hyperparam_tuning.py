...
def objective(trial):
    ...
    n2_gru_layers = trial.suggest_int('n2_gru_layers', 1, 2)
    num_heads = trial.suggest_categorical('num_heads', [2, 4])
    dropout = trial.suggest_float('dropout', 0.0, 0.3)
    learning_rate = trial.suggest_loguniform('learning_rate', 1e-4, 1e-2)
    pos_lambda = trial.suggest_float('pos_lambda', 0.5, 2.0)
    bios_hidden_dim = trial.suggest_categorical('bios_hidden_dim', [32, 64, 128])
    pres_hidden_dim = trial.suggest_categorical('pres_hidden_dim', [32, 64, 128])
    ...
    datasets = {x: PatientDataset(d, y, core=data["padded_tensor_core"], padding_mask=m, padding_mask_core=data["padding_mask_core"], k=k ,notes=n, bios=b, pres=p) for x, d, y, m, n, b, p in
            zip(['train', 'val'], [data["padded_tensor_train"], data["padded_tensor_val"]],
    ...
    val_ap = val_results[1]  # AP for mortality
    return val_ap
